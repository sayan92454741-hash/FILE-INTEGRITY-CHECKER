...
New File Added: When a file appears that did not exist during the previous scan.

Deleted File: When a file present earlier cannot be found in the current scan.

Modified File: When an existing file’s hash value does not match the previously stored hash, indicating changes in its content.
